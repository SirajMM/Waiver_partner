...
class CallFunctionality {
  ...
  Future<void> _onCallAccepted(String? callId) async {
    ...
    homeController.driverState.value = DriverState.loading;
    box.write(BoxKeys.paymentType, data?.paymentType);

    print("Ride Status: ${data?.rideStatus}");

    switch (data?.rideStatus) {
      case "RED":
        homeController.getAndShowOrderDetails(id: data?.rideId ?? "");
        break;
      case "cancelled": // <-- Use string value instead of RideStatus.cancelled
        _player.stop();
        homeController.resetDistance();
        homeController.isTracking = false;
        homeController.rideIsActive = false;
        homeController.driverState.value = DriverState.idle;
        break;
      case "paymentInitiated": // <-- Use string value
        homeController.isTracking = false;
        homeController.rideIsActive = true;
        homeController.driverState.value = DriverState.paymentInitiated;
        break;
      case "completed": // <-- Use string value
        homeController.isTracking = false;
        homeController.rideIsActive = true;
        await homeController.getRidePayment();
