...
void _sendLocationNow() {
  try {
    // Only send if HomeController is registered and available
    if (Get.isRegistered<HomeController>()) {
...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  if (data.rideStatus == "RED" || data.rideStatus == "FRED") {
    ...
    // Only process if HomeController is available
    if (Get.isRegistered<HomeController>()) {
      switch (data.rideStatus) {
        case "CAD":
        case "FCAD":
...
Future<void> main() async {
  ...
  // DON'T initialize LocationTrackingService here for new users
  // It will be initialized in HomeController when needed
  // Only initialize if user is returning and was previously online
  _initializeLocationServiceIfNeeded();
